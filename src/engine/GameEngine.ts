...
export class GameEngine {
  ...
  // Эмитит crowdMilestone при пересечении порогов 50…200 бойцов — ключевой момент в crowd evolution.
  // Потолок толпы — 200 (см. new CrowdManager(scene, 200)), вехи выше недостижимы.
  ...
  constructor(container: HTMLElement, callbacks: GameEngineCallbacks = {}) {
    ...
    // Пробуждение охотника с тыла: короткий толчок тряски + тактильный сигнал
    // (видимая часть телеграфа — HUD-баннер и рык; подписка в HUD.tsx).
  ...
  private setupInputs(): void {
    ...
    this.onKeyDownHandler = (e: KeyboardEvent) => {
      ...
      if (code === 'KeyA' || code === 'ArrowLeft' || key === 'a' || key === 'arrowleft' || key === 'ф') {
        ...
        this.crowd.setFormation('diamond');
      }
    };
