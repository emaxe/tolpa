...
export class LevelGenerator {
  ...
  public static generateLevel(levelNum: number): LevelConfig {
    ...
    // или всю, и могут двигаться/вращаться. Операции: только add (+N) и divide (÷N) —
    // multiply (×N) и mystery убраны по фидбеку игроков (хендлеры остались в GateManager).
    ...
    for (let c = 0; c < coinClusters; c++) {
      ...
      for (let i = 0; i < 4; i++) {
        ...
        coins.push({
          ...
          y: 0.5,
          z: coinZ,
          // Редкие одиночные кристаллы (~8%): дают 25 монет и отличный от обычных
          // фидбек при подборе. Детерминировано: rng сидирован уровнем ниже.
          value: rng() < 0.08 ? 25 : 10,
        });
      }
    }
  ...
  private static generateBoss(levelNum: number): BossData {
    ...
    const baseLaser = { type: 'laser' as const, telegraphTime: Math.max(1.2, 2.0 - telegraphBonus), duration: 1.2, damage: laserDamage, weight: 26 };
  ...
  public static generateEndlessSegment(
    ...
    // На boss-сегментах события НЕ генерируем: толпа заблокирована на арене босса
    // (leaderZ клампится до bossArenaZ-5.5), поэтому метеоритный дождь/засада
    // ложились прямо на неподвижную толпу — бой превращался в нечитаемую мясорубку.
    if (segmentIndex >= 3 && !isBossSegment && rng() < 0.55) {
