...
export class ObstacleManager {
  ...
  /**
   * Громкость звука препятствия относительно лидера толпы (0..1).
   * Камера висит в ~16 м ПОЗАДИ лидера — всё, что ещё в кадре, должно быть
   * слышно: преследователь Охотник (рык пробуждения при dz≈-2.5 и петля погони
   * при dz∈[-16,-1] идут с тыла), хвостовые столкновения, пройденные впритык
   * ловушки. За плоскостью камеры (-18 с запасом на тряску) — тишина.
   * Спереди: максимум при dz <= 4, линейное затухание до радиуса 26 м.
   * (До фикса dz < -1 → 0 глушил ВСЕ тыловые звуки Охотника — погоня была
   * визуальной, а «честное окно реакции на рык» не звучало вообще.)
   */
  private proximityVolume(obsZ: number, leaderZ: number): number {
    const dz = obsZ - leaderZ;
    const REAR_PLATEAU = 14; // полная слышимость до этой глубины за лидером
    const REAR_CUT = 18; // за камерой — тишина
    if (dz < -REAR_CUT || dz > 26) return 0;
    const near = 4;
    if (dz >= -REAR_PLATEAU && dz <= near) return 1;
    if (dz > near) return Math.max(0, 1 - (dz - near) / (26 - near));
    return Math.max(0, 1 - (-REAR_PLATEAU - dz) / (REAR_CUT - REAR_PLATEAU));
  }
  ...
  private checkNearMiss(
    ...
    // Сброс, пока ловушка уничтожена — чтобы следующий проход (например, у
    // качающегося маятника) мог снова дать награду. НЕ гейтим по isHazardActive:
    // для фазовых ловушек (маятник/пресс/шарнирный молот/барьер) опасная фаза —
    // лишь часть цикла, и лидер пересекает плоскость Z чаще всего в НЕопасной фазе.
    // Привязка к isHazardActive съедала момент пересечения и награда молча пропадала.
    ...
    // Пила, секира и таранный шар качаются по X: хитбокс уезжает по дуге каждый кадр,
    // и разовый замер в кадре пересечения плоскости Z ловит случайную фазу — награда/сброс
    ...
    if (obs.type === 'saw_blade' || obs.type === 'axe_pendulum' || obs.type === 'wrecking_ball') {
