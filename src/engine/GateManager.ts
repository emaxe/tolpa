...
export class GateManager {
  ...
  // Высота (м), выше которой основание проёма вертикальных ворот не задевает толпу на
  // земле: честный клиренс — мобы проходят под поднятыми воротами без срабатывания.
  private static readonly GATE_PASS_MAX_Y = 1.35;
  ...
  public update(dt: number, crowd: CrowdManager, particles: ParticleSystem): void {
    ...
    this.gates.forEach((gateVisual) => {
      ...
      // Движение ворот считается ДО дистанционного отсева: при FogExp2 0.008 ворота
      // видны дальше 80 юнитов (~66% контраста на 80м), а прежний порядок замораживал
      // скользящие/вращающиеся ворота вдалеке и заставлял их «стартовать» с фазы 0
      // при подходе лидера. Для ворот позади лидера анимация не нужна — см. return выше.
      ...
      // на Y=0, из-за чего парящие в небе штрафные ворота фантомно вырубали отряд.
      const gateUp = gate.motion === 'vertical' && cy > GateManager.GATE_PASS_MAX_Y;
