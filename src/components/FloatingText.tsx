  ...
  useEffect(() => {
    ...
    const unsubGate = eventBus.on(
      ...
      (data: { op?: string; netChange?: number; comboStreak?: number; x?: number; z?: number; perk?: string | null }) => {
        ...
        if (perk) {
          ...
          } else if (perk === 'oval_buff') {
            spawn(x, z - 1.0, i18n.t('perkOvalGate', 'ОВАЛ: +25%'), 'text-cyan-300 font-bold drop-shadow-[0_0_8px_rgba(34,211,238,0.8)]');
          } else if (perk === 'oval_luck') {
            spawn(x, z - 1.0, i18n.t('perkOvalLuck', 'ОВАЛ: УДАЧА 85%'), 'text-fuchsia-300 font-bold drop-shadow-[0_0_8px_rgba(217,70,239,0.8)]');
          ...
          } else if (perk === 'circle_div') {
            spawn(x, z - 1.0, i18n.t('perkCircleDivGate', 'ФАЛАНГА: -20% ПОТЕРЬ'), 'text-emerald-300 font-bold drop-shadow-[0_0_8px_rgba(16,185,129,0.9)]');
          }
        }
      }
    );
    ...
    const unsubBossEnraged = eventBus.on(
      ...
      (data: { boss?: { z?: number }; x?: number; z?: number }) => {
        ...
        const id = idRef.current++;
        setItems((prev) => [...prev.slice(-24), {
          id, text: i18n.t('bossEnraged'),
          colorClass: 'text-rose-400 font-black text-2xl drop-shadow-[0_0_12px_rgba(244,63,94,1)]',
          x: pos.x, y: pos.y,
        }]);
        window.setTimeout(() => {
          setItems((prev) => prev.filter((it) => it.id !== id));
