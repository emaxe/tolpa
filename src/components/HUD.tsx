...
export const HUD: React.FC<HUDProps> = ({
  ...
  const EVENT_ALERT_MAP: Record<string, { key: string; cls: string }> = {
    ...
    bossAttack_meteors: { key: 'bossAttackMeteors', cls: 'border-orange-500 text-orange-600' },
    bossAttack_shield: { key: 'bossAttackShield', cls: 'border-orange-500 text-orange-600' },
  ...
  useEffect(() => {
    ...
    const unsubBossAttackTelegraph = eventBus.on('bossAttackTelegraph', (data: { type?: string }) => {
      const map: Record<string, string> = {
        slam: 'bossAttack_slam', laser: 'bossAttack_laser', minions: 'bossAttack_minions',
    ...
    const unsubBossStaggered = eventBus.on('bossStaggered', () => {
      showAlert('bossStaggered', undefined, 1600);
    });
  ...
  return (
    <div className="absolute inset-0 pointer-events-none flex flex-col justify-between p-4 select-none">
        ...
        <div className="flex items-center gap-2">
          ...
          {stateManager.getState().settings.showFps && (
            <div className="bg-slate-100/70 border border-slate-300 rounded-lg px-2.5 py-1 text-[11px] text-slate-600 font-mono flex gap-2">
              <span>{i18n.t('fps')}: <strong className="text-emerald-400">{fps}</strong></span>
              <span>{i18n.t('drawCalls')}: <strong className="text-teal-700">{drawCalls}</strong></span>
            </div>
          )}
      ...
      {/* Bottom Action Controls.
          Десктоп: широкая панель по центру низа (адреналин + 6 формаций).
