...
const DIAMOND_X_STEP = 0.55; // полушаг решётки по X
const DIAMOND_Z_STEP = 0.5;  // полушаг решётки по Z (ромб чуть шире, чем глубок)
...
export function getFormationScale(
  ...
  switch (formation) {
    ...
    case 'diamond': {
      const K = Math.ceil((Math.sqrt(Math.max(0, 2 * totalCount - 1)) - 1) / 2);
      const naturalHalf = Math.max(1, K) * DIAMOND_X_STEP;
      return naturalHalf > playableHalfWidth ? playableHalfWidth / naturalHalf : 1;
    }
...
export function calculateFormationOffset(
  ...
  switch (formation) {
    ...
    case 'diamond': {
      // Ромб: концентрические Manhattan-кольца вокруг лидера — плотный фронт; геометрия честная, не спираль-эллипс.
      if (index === 0) {
        target.x = 0;
        target.z = 0;
        return target;
      }
      const K = Math.ceil((Math.sqrt(2 * index + 1) - 1) / 2);
      const within = index - (1 + 2 * (K - 1) * K);
      const q = within;
      let ix = 0;
      let iz = 0;
      if (q < K) {
      ...
      const s = scale ?? getFormationScale(formation, totalCount, playableHalfWidth);
      target.x = ix * DIAMOND_X_STEP * s;
      target.z = iz * DIAMOND_Z_STEP * s;
      return target;
    }
