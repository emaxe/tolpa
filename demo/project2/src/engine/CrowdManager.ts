...
export class CrowdManager {
  ...
  public reset(startingCount: number = 1, startZ: number = 0): void {
  ...
  public spawnMob(preferredType?: MobType, spawnX?: number, spawnZ?: number): MobInstance | null {
    ...
    for (let i = 0; i < this.mobs.length; i++) {
      ...
      if (!mob.alive) {
        mob.alive = true;
        mob.type = mobType;
        mob.x = spawnX !== undefined ? spawnX : this.leaderX + (Math.random() - 0.5) * 2;
        mob.y = 0;
  ...
  public addMobs(count: number): void {
  ...
  public killMobs(count: number, reason: string = 'obstacle'): number {
    ...
    const finalCount = Math.max(1, Math.round(count * (1 - damageReduction)));
    ...
    if (killed > 0) {
      ...
      eventBus.emit('mobsKilled', { count: killed, reason });
    }

    return killed;
  }

  public multiplyMobs(factor: number): void {
  ...
  public update(dt: number, speed: number, steerInput: number, trackWidth: number): void {
    ...
    const halfWidth = trackWidth / 2 - 1.2;
    this.leaderX = clamp(this.leaderX + steerInput * steerSpeed * dt, -halfWidth, halfWidth);
  ...
  private updateMobPositions(dt: number, instant: boolean = false): void {
    ...
    for (let mob of aliveMobs) {
      const offset = calculateFormationOffset(index, totalCount, this.formation);
